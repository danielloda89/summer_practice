...
glob_lat = 0
glob_long = 0
bot = telebot.TeleBot('5597561912:AAELdbRHgRYfPd_5v728bVIPF-DVjrjOmLk')  # Токен бота

user_dict = {}  # {1234567890: {'photo': photo_id, 'caption': some_text}}
"""Не совсем понимаю как работает данная строка, но для работы кода, ее нужно оставлять пустой"""
...
def action(message):
    ...
    Button_Wallet = types.KeyboardButton("💵 Wallet")  # Создание кнопки
    Button_Wallet2 = types.KeyboardButton("💵 Wallet")  # Создание кнопки
    ...
    if message.chat.type == 'private':  # Если личный чат (Бота и человека) 1 на 1
        if message.text == '🚘 Driver':  # Если прожата данная кнопка
            ...
            markup.add(Button_parking_search, Button_Wallet, Back_To_Menu)  # В фундамента клавиатуры добавляются кнопки
            ...
            markup.add(Button_refresh_geo, Button_Wallet2, Back_To_Menu)  # В фундамента клавиатуры добавляются кнопки
...
def test(message):
    ...
    print(type(const))
    print(type(constant))  # принт в консоль
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
